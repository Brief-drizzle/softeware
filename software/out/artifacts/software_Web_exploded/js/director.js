...
$(function() {
    ...
    table.render({
        ...
        cols: [[
            ...
            {title: '审核', width: 60, align: 'center', toolbar: '#directorListBar'}
        ]],
        page: true
    });

    // 监听工具条
    table.on('tool(director-list)', function(obj) {
        var data = obj.data;
        if (obj.event == 'edit') {
            layer.confirm('确定审核吗?', {title: '提示'}, function(index) {
                $.ajax({
                    url: getBasePath() + '/academicExchange/approvalFromDirector.do',
                    type: 'POST',
                    data: {
